...
import os
import re
from development import BASE_SINA_URL, DINGURL
...
JSON_DIR = os.getcwd() + '/raw/sina'
ARCHIVE_DIR = os.getcwd() + '/archives/sina'
READ_FILE = os.getcwd() + '/README.md'
...
def parseHTMLByXPath(content):
    ...
    hrefs = [BASE_SINA_URL + href.strip() for href in hrefs]
    hots = [int(hot.strip().split(' ')[-1])
            for hot in hots]  # 该BASE_SINA_URL处除了热度还会返回大致分类，形如 `剧集 53412536`，前为分类，后为热度
...
def work():
    url = '/top/summary'
    content = getHTML(BASE_SINA_URL + url)
