...
JSON_DIR = os.getcwd()+'/raw/douyin'
ARCHIVE_DIR = os.getcwd()+'/archives/douyin'
READ_FILE = os.getcwd()+'/DOUYIN_README.md'
...
class Douyin:
    ...
    def get_brand_category(self):
        ...
        try:
            ...
            print('get brand category failed')
        return (items, resp)
...
def updateArchive(rank):
    line = '[{title}] {hot}\n'
    lines = [
        line.format(title=v['word'], hot=v['view_count'])
        for v in rank
    ]
    content = '\n'.join(lines)
...
def updateReadme(rank):
    ...
    utils.sync_call_dingding_markdown(development.DINGURL, markText)
    utils.save(READ_FILE, rank)

def worker():
    dy = Douyin()
    items, text = dy.get_hot_search()
    content=updateArchive(items)
